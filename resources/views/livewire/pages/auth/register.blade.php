...
<div>
    @php
        ...
        $githubEnabled = config('services.github.client_id') && config('services.github.client_secret');
        $twitterEnabled = config('services.twitter-oauth-2.client_id') && config('services.twitter-oauth-2.client_secret');
    ...
    <div class="mb-6">
        <div class="flex flex-col space-y-2">
            ...
            @if($twitterEnabled)
            <a href="{{ route('oauth.redirect', ['provider' => 'twitter']) }}" class="flex items-center justify-center px-4 py-2 border border-zinc-300 dark:border-zinc-700 rounded-md shadow-sm text-sm font-medium text-zinc-700 dark:text-zinc-300 bg-white dark:bg-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-700">
                <svg class="h-5 w-5 mr-2" viewBox="0 0 24 24" fill="currentColor">
    ...
    <div class="relative flex items-center justify-center my-6">
        ...
        <div class="px-3 bg-white dark:bg-zinc-900 text-sm text-zinc-500 dark:text-zinc-400">{{ __('Or register with your name and email') }}</div>
        <div class="border-t border-zinc-300 dark:border-zinc-700 w-10"></div>
    </div>
    @endif
